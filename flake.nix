{
  ...
  inputs = {
    ...
    vim-rescript.url = "github:rescript-lang/vim-rescript";
    vim-rescript.flake = false;

    vim-gitgutter.url = "github:airblade/vim-gitgutter";
    vim-gitgutter.flake = false;
  ...
  outputs = { self, nixpkgs, neovim, rnix-lsp, unison-nix, ... }@inputs:
    let
      system = "x86_64-linux";
      plugins = [
        "vim-gitgutter"
      ...
      allPkgs = lib.mkPkgs {
        inherit nixpkgs;
        cfg = { };
        overlays = [ unison-nix.overlay pluginOverlay externalBitsOverlay ];
      };
